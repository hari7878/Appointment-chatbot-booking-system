...
from data_preprocessing.config import TABLE_DEFINITIONS
import os
...
def create_tables(conn):
    ...
    # ... (Existing implementation - it will now pick up the new table defs) ...
    if not conn:
    ...
    try:
        ...
        for table_name, definition in TABLE_DEFINITIONS.items():
            # Check if it's a CREATE TABLE or CREATE INDEX statement
            if definition.strip().upper().startswith("CREATE"):
                 logging.debug(f"Executing schema for: {table_name}")
                 cursor.execute(definition)
            else:
                 logging.warning(f"Skipping definition for '{table_name}' as it doesn't start with CREATE: {definition[:50]}...")

        conn.commit()
        logging.info("Tables/Indices checked/created successfully.")
        return True
    except sqlite3.Error as e:
        logging.error(f"Error creating tables/indices: {e}")
        try:
            conn.rollback()
